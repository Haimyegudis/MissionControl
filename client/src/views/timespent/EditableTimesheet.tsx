// Editable timesheet: type hours directly into the grid to log work — no
// dialog. Empty/zero cells for the signed-in user become inline inputs;
// filled cells show the day total and open a small "+ add" popover for a
// second worklog. Read-only (plain text, no inputs) whenever the scope
// bar's user picker points at someone other than the signed-in user, since
// the server always logs as the session user.
...
export function EditableTimesheet({
  ...
  // Cells whose empty-input Escape was pressed. Must be marked BEFORE the
  // synchronous .blur() call fires, since the native blur event runs before
  // React flushes the drafts-state update — onBlur would otherwise read a
  // stale (pre-delete) draft from its closure and commit a worklog the user
  // just cancelled. Worklogs can't be deleted in-app, so that bug would be
  // irreversible.
  ...
  const commit = async (
    ...
    let seconds = parseCellInput(trimmed);
    if (seconds === null) {
    ...
    if (existingDaySeconds > 0) {
      const clamped = clampToDayCap(existingDaySeconds, seconds, MAX_CELL_SECONDS);
      if (clamped <= 0) {
        pushToast({ title: 'Day already at the 24h cap.', body: '', severity: 'error' });
        close();
        return;
      }
      if (clamped < seconds) {
        pushToast({ title: 'Clamped to the 24h daily cap.', body: '', severity: 'error' });
        seconds = clamped;
      }
    }
    if (seconds < 60) {
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        // Mark cancelled BEFORE blur — the native blur event
                        // fires synchronously, ahead of React's state flush,
                        // so onBlur must consult this ref rather than the
                        // (still-stale) drafts state.
