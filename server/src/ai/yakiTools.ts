...
Return plain text (no JSON, no markdown fences).`;
  try {
    const raw = await ctx.runCli(reasoningPrompt, ctx.model || 'claude-sonnet-5[1m]', ctx.signal);
...
- 1-3 short paragraphs of plain text.`;
  try {
    const raw = await ctx.runCli(synthPrompt, ctx.model || 'claude-sonnet-5[1m]', ctx.signal);
    return { answer: String(raw || '').slice(0, 3000) };
  } catch (err) {
    return { error: err instanceof Error ? err.message : String(err) };
  }
